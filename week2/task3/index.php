<?php
    ...
    // Create posts using the class PostSeeder
    $posts = PostSeeder::seed(); // Calling static method

    // Add comments to posts
    $posts['Post1']->addComment('Lani', 'Amy! Lovely cat!');
                    ...
                    <?php foreach($posts as $post) { ?>
                        <div class="post border border-dark-subtle rounded-end my-3 p-2">
                            ...
                            <?php foreach($post->getComments() as $comment) { ?>
                                <?= $comment->getUser() ?>:<?= $comment->getMessage() // Calling Methods ?><br>
                            <?php } ?>
                            </div>
