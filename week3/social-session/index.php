<?php
/**
 * @directory: social-session
 * @description: Implements the comments feature for posts on the website using sessions.
 *               Users can add comments to existing posts, and these comments are
 *               displayed below the respective posts. The session is used to store
 *               and retrieve the posts data, ensuring the comments are preserved
 ...
 */
    ...
    use wad\PostSeeder;
     ...
     // Sessions are used to store data across multiple page requests.
    session_start();
    ...
    if (isset($_SESSION['posts'])) {
        // indicating a comment was added in add_comment.php
        $posts = $_SESSION['posts'];
    } else {
        // seed new posts for the first visit
                    ...
                    <?php foreach($posts as $index => $post) { ?>
                        <div class="post border border-dark-subtle rounded-end my-3 p-2">
                            ...
                            <div class="container mt-3">
                                <!-- Sending comment input
                                    required input data: post index, comment
                                -->
                                <form method="get" action="add_comment.php" class="d-flex align-items-center">
                                    <img src="https://github.com/mdo.png" alt="User Avatar" width="32" height="32" class="rounded-circle me-2">
                                    
                                    <!-- Sending post index as a hidden input with the key 'index' -->
                                    <input type="hidden" name="index" value="<?= $index ?>">
                                    <div class="form-floating flex-grow-1 me-2">
                                        <!-- sending comment from user input with the key `comment` -->
